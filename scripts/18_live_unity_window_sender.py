...
def draw_overlay(frame, candidates, pair, pair_score, packet, raw_detection, fps_est, sent_total, red_pixel_count=0):
    out = frame.copy()
    h, w = out.shape[:2]
    ...
    # ---- KIRMIZI olcum gostergesi: SOL-ALT kose, mavi/siyah kutu ----
    # Yesil overlay'e dokunmaz; ayri kosede durur.
    red_label = f"RED: {red_pixel_count}"
    rx, ry = 20, h - 25
    (tw, th), _ = cv2.getTextSize(red_label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
    cv2.rectangle(out, (rx - 8, ry - th - 10), (rx + tw + 8, ry + 8), (0, 0, 0), -1)
    cv2.putText(out, red_label, (rx, ry), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                (255, 80, 0), 2, cv2.LINE_AA)  # mavi (BGR: B=255)
...
def main():
    ...
    # ---- KIRMIZI (emergency) algilama -- ASAMA 1: sadece olcum ----
    # Kirmizi HSV'de iki uctadir (~0 ve ~180). Parlak/doygun kirmizi icin.
    ...
    with mss.MSS() as sct:
        ...
        try:
            while True:
                ...
                # ===== KIRMIZI OLCUM (ASAMA 1) -- yesil takibe dokunmaz =====
                # Ayni frame uzerinde bagimsiz kirmizi piksel sayimi.
                ...
                if time.time() - last_print_time >= 1.0:
                    ...
                    # ===== KIRMIZI OLCUM LOGU (ASAMA 1) -- ayri satir =====
                    print(f"[KIRMIZI] piksel={red_pixel_count}")
